...
def avg_within(sense: str) -> float:
    ...
    pairs = [cosine(vs[i], vs[j]) for i, j in itertools.combinations(range(len(vs)), 2)]
...
rows = []
for sense in sentences_by_sense:
    rows.append({"sense_a": sense, "sense_b": sense, "kind": "within",
                 "mean_cos": round(avg_within(sense), 3)})
for sense_a, sense_b in itertools.combinations(sentences_by_sense, 2):
    rows.append({"sense_a": sense_a, "sense_b": sense_b, "kind": "across",
...
apple_positions_p = [i for i, t in enumerate(toks_p) if TARGET_WORD in t.lower()]
...
print(f"Long passage: {len(toks_p)} tokens, '{TARGET_WORD}' at positions {apple_positions_p}")
print(f"Short query:  {len(toks_q)} tokens, '{TARGET_WORD}' at position  {apple_pos_q}")
...
# %%
two_senses = "Steve Jobs founded Apple in 1976. He often ate an apple at his desk."

toks_s, vecs_s, _ = get_token_vectors(two_senses, model_name=MODEL_NAME)
apple_positions_s = [i for i, t in enumerate(toks_s) if TARGET_WORD in t.lower()]
assert len(apple_positions_s) == 2, f"Expected 2 'apple' tokens, got {len(apple_positions_s)}"

vec_company = vecs_s[apple_positions_s[0]]
vec_fruit = vecs_s[apple_positions_s[1]]

print(f"Tokens around company-apple (idx {apple_positions_s[0]}): "
      f"{toks_s[max(0, apple_positions_s[0]-3):apple_positions_s[0]+4]}")
print(f"Tokens around fruit-apple   (idx {apple_positions_s[1]}): "
      f"{toks_s[max(0, apple_positions_s[1]-3):apple_positions_s[1]+4]}")

intra_sentence_cos = cosine(vec_company, vec_fruit)
print(f"\ncosine(company-apple, fruit-apple) within same sentence: {intra_sentence_cos:.3f}")
...
summary = pd.DataFrame([
    {"comparison": "intra-sentence (company ↔ fruit, same sentence)",  "cosine": round(intra_sentence_cos, 3)},
    {"comparison": "cross-sentence (company ↔ fruit, mean from 1b)",   "cosine": round(cross_sentence_cos, 3)},
    {"comparison": "within-sense reference (company ↔ company)",       "cosine": round(within_company, 3)},
    {"comparison": "within-sense reference (fruit ↔ fruit)",           "cosine": round(within_fruit, 3)},
])
summary
